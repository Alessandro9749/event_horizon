...
SCRIPT_VERSION="EventHorizon 1.3.1"
...
show_help() {
    cat << 'EOF'
================================================================================
                        EVENT HORIZON - Archive Extractor
================================================================================

USAGE:
    bash event_horizon.sh [OPTIONS] [FILE]

DESCRIPTION:
    Event Horizon is a versatile archive extraction tool that supports multiple
    archive formats and provides interactive prompts for a user-friendly experience.
    It automatically detects archive types, handles password-protected archives,
    and manages destination folder conflicts.

OPTIONS:
    -h, --help          Show this help message and exit
    -v, --version       Show version information and exit
    -y, --yes           Auto-confirm all prompts (useful for scripting)
    --allow-root        Allow running as root (default: re-exec as normal user)
    --rename            Prompt to rename destination folder after extraction

SUPPORTED ARCHIVE FORMATS:
    Standard Archives:
        .zip            ZIP archive
        .rar, .rar4     RAR archive (requires 7z or unrar)
        .7z             7-Zip archive
        .tar            TAR archive
    ...
    Single-file Compression:
        ...
        .bz2, .bzip2    Bzip2 compressed file
        .xz             XZ compressed file
        .lzma           LZMA compressed file
        .zst, .zstd     Zstandard compressed file
        .lz4            LZ4 compressed file

    LZ4-wrapped Archives:
        .zip.lz4        ZIP + LZ4
        .tar.lz4        TAR + LZ4
        .rar.lz4        RAR + LZ4

DEPENDENCIES:
    The script will prompt to install missing dependencies automatically.
    Required tools vary by archive type:
    
    - unzip, file       For ZIP archives
    - 7z (p7zip-full)   For 7z and RAR archives
    - tar, gzip, bzip2  For TAR and compressed formats
    - xz-utils          For XZ/LZMA formats
    - zstd              For Zstandard compression
    - lz4               For LZ4 compression
    - wimtools          For WIM images

INTERACTIVE PROMPTS:
    During extraction, you may encounter these prompts:
    
    Input Selection:
        (a) Select from found archives in current directory
        (b) Paste a custom file path
    
    Extraction Mode (for single-file archives):
        (h) Extract here - Extract to current directory
        (c) Classic      - Extract into a named folder
    
    Destination Conflict:
        (o) Overwrite    - Replace existing files/folders
        (r) Rename       - Choose a new name
        (a) Abort        - Cancel the operation
    
    Password-protected Archives:
        The script will prompt for passwords up to 3 attempts

EXAMPLES:
    # Interactive mode (default)
    bash event_horizon.sh

    # Extract specific file with auto-confirm
    bash event_horizon.sh -y archive.zip

    # Show version
    bash event_horizon.sh --version

    # Allow root execution
    bash event_horizon.sh --allow-root archive.tar.gz
...
FUNCTIONS REFERENCE:
    ...
    is_termux()
        ...
        Returns: 0 (true) if Termux, 1 (false) otherwise
    
    choose_tmp_dir()
        Selects appropriate temporary directory.
        Order of preference: ./temp, ./templates, current directory
        Returns: Path to temp directory (stdout)
    
    make_temp_file()
        Creates a unique temporary file.
        Returns: Path to temp file (stdout)
    ...
    cleanup()
        Removes temporary files/directories on script exit.
        Called automatically via trap on EXIT, INT, TERM signals.
    
    abort([message])
        Prints optional message and exits with status 1.
        Usage: abort "Error message here"
    
    ------------------ Package Management ------------------
    
    cmd_to_pkg(command)
        Maps command names to package names for the current distro.
        Supports: Termux (pkg), Debian/Ubuntu (apt-get)
        Usage: pkg_name=$(cmd_to_pkg "7z")
    
    install_missing_deps(pkg1 pkg2 ...)
        Installs missing packages using available package manager.
    ...
    abs_path(path)
        Returns absolute path for an existing file/directory.
        Tries: realpath, readlink -f, python3, fallback join
        Usage: full_path=$(abs_path "./file.txt")
    
    suggest_unique_dest(base_name)
        Generates a unique destination name by appending numbers.
        Usage: new_name=$(suggest_unique_dest "folder")
        Example: "folder" -> "folder_1" if "folder" exists
    ...
    read_char_prompt(var_name "prompt")
        Reads a single character from user.
        Usage: read_char_prompt ANSWER "Continue? (y/n) "
    
    read_line_prompt(var_name "prompt")
        Reads a full line of text from user.
        Usage: read_line_prompt NAME "Enter filename: "
    
    read_secret_prompt(var_name "prompt")
        Reads password/secret text (hidden input).
        Usage: read_secret_prompt PASS "Password: "
    
    prompt_overwrite_rename_abort_file(path)
        Handles file conflicts with overwrite/rename/abort options.
        Returns: Final path to use (stdout)
        Usage: out=$(prompt_overwrite_rename_abort_file "./output.txt")
    
    ensure_dest_folder(path)
        Creates destination folder, handles conflicts.
        Sets global: DEST_FOLDER, OVERWRITE
        Usage: ensure_dest_folder "./output_folder"
    
    ------------------ Spinner Functions ------------------
    
    spinner()
        Internal: Displays animated dots while processing.
    
    start_spinner("message")
        Shows a spinner with optional message.
        Usage: start_spinner "Extracting files"
    ...
    strip_archive_ext(filename)
        Removes archive extension from filename.
        Preserves other dots in the name.
        Usage: base=$(strip_archive_ext "photo.backup.tar.gz")
        Result: "photo.backup"
    
    flatten_single_top_dir(dest)
        If folder contains single subdirectory, moves contents up.
        Usage: flatten_single_top_dir "./extracted"
    
    detect_single_top_file(ext real_type temp_file)
        Detects if archive contains exactly one top-level file.
        Returns: Filename (stdout) or empty if multiple files
        Usage: single=$(detect_single_top_file "zip" "Zip archive" "./file.zip")

EXIT CODES:
    0   Success
    1   Error/Abort (missing deps, user abort, extraction failure)
...
cmd_to_pkg() {
    ...
    # NOTE: this script is intended to work on Debian/Ubuntu (apt), Arch (pacman) and Termux.
    local cmd="$1"
...
install_missing_deps() {
    ...
    # Installs packages using Termux pkg, Arch pacman, or Debian apt-get.
    local pkgs=("$@")
    if ((${#pkgs[@]}==0)); then
        return 0
    fi
    ...
    if command -v pacman >/dev/null 2>&1; then
        ...
        # Show pacman output (users expect progress + prompts if any)
    ...
    echo "No supported package manager found (need Termux pkg, pacman, or apt-get)." >&2
    return 1
}
...
if [ "${#MISSING[@]}" -ne 0 ]; then
    ...
    if [[ "${ASSUME_YES:-0}" -eq 1 ]]; then
        ...
        while true; do
            ANSWER=""
            read_line_prompt ANSWER "Install them? (y/n)" || abort "Cannot read input. Re-run with --yes to auto-install."
            ANSWER="${ANSWER,,}"
            case "$ANSWER" in
                y|yes)
